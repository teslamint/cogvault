...
여러 에이전트(Codex, Codex, Gemini 등)가 같은 맥락을 참조할 수 있도록 정본 위치를 분리한다.
...
- `AGENTS.md`에만 있는 결정은 정본으로 간주하지 않는다.
...
**의도적으로 다르게 한 것**:
- sage-wiki는 API 직접 호출만 지원. 우리는 **LLM 어댑터 패턴** — Codex CLI subprocess로 구독 요금제 활용 가능 (v0.2).
...
- **LLM 어댑터**: `llm/Codex.go` — `Codex --print` CLI를 subprocess로 호출. **stdin pipe** 필수 (OS ARG_MAX 제한 회피). stderr 캡처로 인증 실패 진단. `--output-format json` 사용 검토. CLI 인터페이스가 Anthropic에 의해 변경될 수 있으므로 `llm/anthropic.go` (API 직접 호출)를 fallback으로 함께 구현.
...
| Codex CLI `--print` 인터페이스 변경 (v0.2) | Codex 어댑터 동작 불가 | anthropic.go API 직접 호출 fallback. CLI 출력 파싱 최소화, JSON 모드 우선. |
...
- [ ] SPEC.md, DESIGN.md, AGENTS.md를 프로젝트 루트에 배치
