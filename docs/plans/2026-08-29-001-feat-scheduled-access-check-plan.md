...
|---|---|---|
| S1 — verify configured ingest paths | U1 → U3 | `TestAccessCheckConfiguredSurfaces` records enumeration of two configured roots, including a new settle-window file, and proves no runtime bootstrap. Covers S1. |
...
Test scenarios:
  happy: temporary wiki and database-parent probes succeed; two configured source roots are each enumerated exactly once; accepted top-level files get one bounded read; sentinels are absent afterward
...
Steps:
  1. Add failing command tests for two-root enumeration, config-only behavior, settle-window access, success output, positional-argument rejection, cleanup, filters, descriptor identity, exact readback, permission diagnostics, and combined errors.
  ...
  4. Run `gofmt` on the new and modified Go files. Run `go test ./cmd/cogvault -run TestAccessCheck`, `! rg -n '(/Volumes|exec\\.Command.*mount)' cmd/cogvault/access_check.go`, and `go test -race ./...`.
  5. Commit: `feat(cli): verify configured ingest filesystem access`
Acceptance: the targeted tests pass; every configured source root is recorded exactly once; no database, sidecar, or schema asset appears; the static mount-discovery guard and full race suite pass; no sentinel remains.
...
Steps:
  1. Add a failing shell test with fake platform, GUI-domain, launchctl, codesign, and CogVault commands plus real plutil assertions. Cover preflight rejection, signals, binary replacement, private modes, PID completion, bootout failure, and recovery commands.
  ...
  4. Run `bash -n` on both scripts. Run the shell test and confirm every success, failure, timeout, path-escaping, and cleanup assertion passes.
  5. Commit: `feat(launchd): verify scheduled configured-path access`
Acceptance: `bash scripts/check-scheduled-access_test.sh` passes and `bash -n scripts/check-scheduled-access.sh scripts/check-scheduled-access_test.sh` exits zero.
...
Steps:
  ...
  3. Search all three documents for broad TCC claims, machine-specific identities, and unconfigured-folder instructions. Remove each occurrence.
