...
if [[ $1 == -d ]]; then echo 'designated => identifier "dev.tmint.cogvault"' >&2; fi
EOF
  cat >"$dir/bin/shasum" <<'EOF'
#!/bin/bash
file=${@: -1}; c="$FAKE_STATE/shasum-count"; n=0; [[ -f $c ]] && read -r n <"$c"; n=$((n+1)); echo "$n" >"$c"
if [[ ${FAKE_MODE:-} == identity-change && $n -gt 1 ]]; then echo "changed  $file"; else echo "stable  $file"; fi
EOF
...
case $cmd in
  ...
  kickstart)
    ...
    [[ ${FAKE_MODE:-} == missing-marker ]] || echo 'configured ingest access check passed' >"$out"
    : >"$err"
    ...
    ;;
  bootout)
    [[ ${FAKE_MODE:-} == retained-loss ]] && { rm -rf "$FAKE_STATE/private"; exit 1; }
...
for mode in invalid-signature second-run-signature-failure identity-change missing-marker timeout; do
  run_case "$mode" "$mode"
  [[ $CASE_STATUS -ne 0 ]] || fail "$mode unexpectedly passed"
done
for mode in second-run-signature-failure identity-change missing-marker timeout; do
...
run_case retained-loss retained-loss
[[ $CASE_STATUS -ne 0 && ! -d "$CASE_DIR/state/private" ]] || fail "retained-loss injection was not observed"
...
PY
  ...
  grep -q '^bootout ' "$dir/state/transcript" || fail "$sig did not boot out temporary job"
done
...
if [[ ${SKIP_MUTATIONS:-0} == 0 ]]; then
  ...
  set +e; SKIP_MUTATIONS=1 HARNESS="$TMP/no-second-run.sh" bash "$0" >/dev/null 2>&1; mutation_status=$?; set -e
  ...
  set +e; SKIP_MUTATIONS=1 HARNESS="$TMP/changed-label.sh" bash "$0" >/dev/null 2>&1; mutation_status=$?; set -e
